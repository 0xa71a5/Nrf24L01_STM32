...
#define NRF_CSN_HIGH()      GPIOC->BSRR = GPIO_Pin_4;//GPIO_SetBits(GPIOC, GPIO_Pin_4)
#define NRF_CSN_LOW()       GPIOC->BRR = GPIO_Pin_4;//GPIO_ResetBits(GPIOC, GPIO_Pin_4)		        //csn置低
#define NRF_CE_HIGH()	      GPIOA->BSRR = GPIO_Pin_4;//GPIO_SetBits(GPIOA,GPIO_Pin_4);//
#define NRF_CE_LOW()	      GPIOA->BRR = GPIO_Pin_4;//	GPIO_ResetBits(GPIOA,GPIO_Pin_4);//	      //CE置低
#define NRF_Read_IRQ()		  GPIO_ReadInputDataBit( GPIOC, GPIO_Pin_5)//(GPIOC->IDR & GPIO_Pin_5) != (uint32_t)Bit_RESET//  //中断引脚
