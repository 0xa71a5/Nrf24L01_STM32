...
#include "other.h"
 ...
 u8 TX_BUF[32];		//发射数据缓存
extern u32 millis;
int  TX_PLOAD_WIDTH=32;  //发射数据通道有效数据宽度0~32Byte 
...
void SPI_NRF_Init(void)
{
 ...
 /*使能SPI1时钟*/
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE);
   ...
   /*配置 SPI_NRF_SPI的 SCK,MISO,MOSI引脚 B13 B14 B15 */
...
void NRF_RX_Mode_With_Frequency(u8 *rx_addr,u8 freq)
{
   ...
   SPI_NRF_WriteReg(NRF_WRITE_REG+EN_AA,0x03);    //使能通道0的自动应答    

   SPI_NRF_WriteReg(NRF_WRITE_REG+EN_RXADDR,0x03);//使能通道0的接收地址    
	
   SPI_NRF_WriteReg(NRF_WRITE_REG+RF_CH,freq);      //设置RF通信频率    
...
{  
   ...
   SPI_NRF_WriteBuf(NRF_WRITE_REG+TX_ADDR,tx_addr,TX_ADR_WIDTH);    //写TX节点地址 

   SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P0,tx_addr,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK   

   SPI_NRF_WriteReg(NRF_WRITE_REG+EN_AA,0x03);     //使能通道0 1的自动应答    

   SPI_NRF_WriteReg(NRF_WRITE_REG+EN_RXADDR,0x03); //使能通道0 1的接收地址  
...
{  
   ...
   SPI_NRF_WriteReg(NRF_WRITE_REG+EN_AA,0x00);     //禁止自动应答  
